...
; Note:	BW6502 BIOS uses Page Zero locations from $E0 - $??

;******************************************************************************
; BW6502 BIOS Page Zero definitions
;******************************************************************************

; Note: BW6502 BIOS uses Page Zero locations from $E0 - $FF
; C02Monitor uses Page Zero locations from $B0 - $DF

BW6502_PG_ST    = $E0           ;Start of Page Zero usage
r0				= BW6502_PG_ST     ; 2 bytes
r0L				= r0            ; low byte
r0H				= r0+1         ; high byte
r1				= r0+2         ; 2 bytes
r1L				= r1            ; low byte
...
r5L				= r5            ; low byte
r5H				= r5+1         ; high byte
lcd_addr		= r5+2           ; 1 byte, lcd busy flag and address
; in_cnt			= lcd_addr+1	        ; 1 byte
; in_tail			= in_cnt+1	        ; 1 byte
in_wptr         = lcd_addr+1
; in_head			= in_tail+1		    ; unused
in_rptr         = in_wptr+1
kb_flags		= in_rptr+1	        ; 1 byte
kb_modifiers 	= kb_flags+1	        ; 1 byte
PGZERO_LAST     = kb_modifiers+1     ; BW6502 BIOS Page Zero ends at $F1
; $F2 - $FF are available for additional BIOS functions

; MON_PZ_ST       = $B0               ; Start of Page Zero usage (C02 Monitor)

; PROMPTL         = MON_PZ_ST+22      ; Prompt string address
; PROMPTH         = MON_PZ_ST+23
; SRCL            = MON_PZ_ST+24      ; Source address for memory operations
; SRCH            = MON_PZ_ST+25
; TGTL            = MON_PZ_ST+26      ; Target address for memory operations
; TGTH            = MON_PZ_ST+27
; LENL            = MON_PZ_ST+28      ; Length address for memory operations
; LENH            = MON_PZ_ST+29

; OPXMDM          = MON_PZ_ST+38      ; Saved Opcode/Xmodem Flag variable
; PTRL            = MON_PZ_ST+42      ; Data pointer lo byte
; PTRH            = MON_PZ_ST+43      ; Data pointer hi byte
; BLKNO           = MON_PZ_ST+44      ; Block number
